...
def main(args):
    ...
    for step in trange(args.n_steps):
        ...
        if (step+1) % args.eval_period == 0:
            ...
            print("Saving image")
            img = env.render(mode="rgb_array")
            save_img(img, args.env_name, step+1)
            frames.append(img)
            print("Saved!")

    print("Saving gif")
    save_frames_as_gif(frames, path='./', filename=f'iql_{args.env_name}.gif')
    print("Saved!")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-steps', type=int, default=250000) #10**6)
